     ...
     */
		function set($pagesize=20,$total,$current=false) {
			...
			if ($this->tpage > 1) {
				...
				if ($current>10) {
					$this->output_pre10 = '<a href='.$this->file.'?'.$this->pvar.'='.($current-10).($this->varstr).' title="上十页">&lt;&lt;&lt;</a>&nbsp;';
				}
				if ($current>1) {
					$this->output_pre = '<a href='.$this->file.'?'.$this->pvar.'='.($current-1).($this->varstr).' title="上一页">上一页</a>&nbsp;';
				}
				...
				for ($i=$start; $i<=$end; $i++) {
					if ($current==$i) {
						$this->output_page.='<font color="red">'.$i.'</font>&nbsp;';    //输出当前页数
