...
switch ($action)
{
    case "copty_goods_id_to_user_id": //复制goods_id到user_id
        if($_POST)
        {
            ...
            if( ! $goods_id || ! $user_id )
            {
                exit('<script>alert("商品与用户id不能为空");window.location.href="goods_search.php?action=copty_goods_id_to_user_id"</script>');
            }
...
//		}
	...
	break;
    case "goods_export": //原来goods.php 的export功能的逻辑
		$store_id = (int)$_INPUT['store_id'];
		$status = (int)$_INPUT ['status'];
	...
	case "order_management":
		...
		{			
			...
			$v['order_is_show'] = ($order_info['wait_pay']>0 and $order_info['wait_sign']>0)?"1":"0";
		}		
		...
		$fp = new FirePHP;
    	$fp->fb($goods['prices_data_list']);
		
	    break;
    case "goods_list": //原来goods.php list的逻辑
		$status = (int)$_INPUT ['status'];
		...
		$page_obj = new show_page ();
		$show_count = 20;
		$page_obj->setvar ( array ("status" => $status,"begin_time" => $begin_time,"end_time" => $end_time,"show" => $show,"type_id" => $type_id,"keyword" => $keyword,'is_black'=>$is_black,'action'=>'goods_list') );
		$page_obj->set ( $show_count, $total_count );		
		$list = $task_goods_obj->get_goods_list(false, $where, "goods_id DESC", $page_obj->limit());
		//print_r($list);
		$task_seller_obj = POCO::singleton('pai_mall_seller_class');
    ...
    case "export":
        ...
        $_INPUT ['is_black'] = isset($_INPUT ['is_black']) ? (int)$_INPUT ['is_black'] : 0;
        $_INPUT ['type_id'] = in_array((int)$_INPUT ['type_id'], array(3, 5, 12, 31, 40, 41, 43, 42)) ? (int)$_INPUT ['type_id'] : 31;
        $type_id = (int)$_INPUT ['type_id'];
        ...
        {
            $val['type_name'] = $type_list_name[$val['type_id']]['name'];
            $val['status_name'] = isset($val['goods_status']) ? $status_name[$val['goods_status']] : $status_name[$val['status']];
            ...
            $location_id = explode(',', $goods_data['goods_data']['location_id']);
            $location_name = '';
            foreach ($location_id as $val_de)
            {
                $location_name .= ($location_name ? "\n" : "") . ($val_de ? get_poco_location_name_by_location_id($val_de) : ($val_de === 0 ? "全国" : ""));
            }
            $prices_list_de = $val['priceslist'] ? unserialize($val['priceslist']) : unserialize($val['prices_list']);
            $val['prices_list_de'] = array();
            if ($prices_list_de)
            {
                foreach ($prices_list_de as $key_de => $val_de)
            ...
            {
                $ex_val[] = str_replace("<br>", "", $val['goods_att']['219']);
                $ex_val[] = str_replace("<br>", "", $val['goods_att']['220']);
                $ex_val[] = str_replace("<br>", "", $val['goods_att']['250']);
                $ex_val[] = $val['goods_att']['258'];
                $ex_val[] = $val['goods_att']['259'];
                $ex_val[] = $val['goods_att']['260'];

                $task_supplier_obj = POCO::singleton('pai_mall_supplier_class');
                $supplier_info = $task_supplier_obj->get_supplier_info_by_goods_id($val['goods_id']);

                $ex_val[] = $supplier_info['supplier_user_id'];
                $ex_val[] = $supplier_info['supplier_user_name'];
                $ex_val[] = $supplier_info['supplier_user_cellphone'];

            }
            else if ($type_id == 43)
            {
                $ex_val[] = str_replace("<br>", "", $val['goods_att']['278']);
                $ex_val[] = $val['goods_att']['357'];
            }
    ...
    default:
        ...
        // 商品状态 0 未审核 1 通过 2未通过 3删除
        $status = (int)$_INPUT ['status'];
        // 修改审核状态,0.无 1.未审核 2.审核通过 3.未通过
        $edit_status = (int)$_INPUT ['edit_status'];
        // 1上架,2下架
        $is_show = (int)$_INPUT ['is_show'];
        // 是否黑名单,0不是 1黑名单
        $is_black = isset($_INPUT ['is_black']) ? (int)$_INPUT ['is_black'] : 0;
        // 判断 商品品类ID 是否在内定范围里，如果不在默认是31
        $type_id = in_array((int)$_INPUT ['type_id'], array(3, 5, 12, 31, 40, 41, 43, 42)) ? (int)$_INPUT ['type_id'] : 31;
        // 用户ID
        ...
        if ($type_id != 5)
        {
            $price_list_arys = $task_goods_obj->get_goods_prices_scope_list($type_id);
            $price_list_val = (int)$_INPUT['prices_list'];

            foreach ($price_list_arys as $k => $v)
            {
                if ($v['id'] == $price_list_val)
                {
                    $price_list_arys[$k]['selected_one'] = 1;
                }
            }
        }
        ...
        foreach ($list as $key => $val)
        {
            ...
            $location_id = explode(',', trim($goods_data['goods_data']['location_id'], ','));
            $location_name = '';
            foreach ($location_id as $val_de)
            {
            ...
            if ($goods_data['goods_data']['edit_status'] == 2)
            {
                $list[$key]['edit_status_text'] = '修改审核通过';
            }
            else if ($$goods_data['goods_data']['edit_status'] == 3)
            {
                $list[$key]['edit_status_text'] = '修改审核不通过';
            }
            else if ($goods_data['goods_data']['edit_status'] == 1)
            {
                $list[$key]['edit_status_text'] = '';
            }
        ...
        $tpl = new SmartTemplate (TASK_TEMPLATES_ROOT . "goods_list_search.tpl.htm");
        ...
        $tpl->assign('audit_time_s', $_INPUT['audit_time_s']);
        $tpl->assign('audit_time_e', $_INPUT['audit_time_e']);
        ...
        $tpl->assign('ot_label', $_INPUT['ot_label']);

        $tpl->assign('name_19', $_INPUT['name_19']);
        $tpl->assign('yp_background', $_INPUT['yp_background']);
        $tpl->assign('yp_can_photo', $_INPUT['yp_can_photo']);
        $tpl->assign('yp_lighter', $_INPUT['yp_lighter']);
        $tpl->assign('yp_other_equitment', $_INPUT['yp_other_equitment']);
        ...
        $tpl->assign('ms_experience', $_INPUT['ms_experience']);
        $tpl->assign('ms_certification', $_INPUT['ms_certification']);
        $tpl->assign('ms_forwarding', $_INPUT['ms_forwarding']);
        $tpl->assign('week', $_INPUT['week']);
        ...
        $tpl->assign('end_time', $end_time);
        //活动
        $tpl->assign('is_official', $_INPUT['is_official']);
        $tpl->assign('huo_add_time_s', $_INPUT['huo_add_time_s']);
        $tpl->assign('huo_add_time_e', $_INPUT['huo_add_time_e']);
        break;
}
