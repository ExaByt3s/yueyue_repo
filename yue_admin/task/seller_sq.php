...
switch($action)
{
    ...
    case 'check':
        ...
        $basic_one = $mall_basic_check_obj->get_info($basic_id);
        if($basic_one['status'] !=0 )
        {
           echo "<script>alert('状态不是未审核不能改');window.open('seller_sq.php?status=0','_self');</script>";
           exit;
        }
        ...
        if($status == 1)
        {
            ...
            if( $rs && empty($seller_info) )
            {
                ...
                $_add_data['basic_type'] = $basic_type;
                $_add_data['name'] = get_user_nickname_by_user_id($user_id);
                $_add_data['basic_id'] = $basic_id;
                $_add_data['avatar'] = get_user_icon($user_id, 165);
                
                $mall_seller_obj->add_seller($_add_data);
            }
            ...
            if($rs > 0)
            {
                ...
                echo "<script>alert('操作成功');window.open('seller_sq.php?status=0','_self');</script>";
                exit;
            }else
            {
                echo "<script>alert('操作失败');window.open('seller_sq.php?status=0','_self');</script>";
                exit;
            }
        }
    ...
    default://商家申请列表
        ...
        if( $keyword )
        {
            $where .= " and (user_id = "."'".(int)$keyword."'"." or phone like '".pai_mall_change_str_in($keyword)."%' or person_id_card like '".pai_mall_change_str_in($keyword)."%' or company_id_card like '".pai_mall_change_str_in($keyword)."%') ";
        }
		...
		foreach($list as $key => &$val)
		{
            ...
            $val['user_name'] = get_user_nickname_by_user_id($val['user_id']);
            $val['operator_name'] = $val['operator_id'] ? get_user_nickname_by_user_id($val['operator_id'])."</br>[{$val['operator_id']}]" : '批量';
