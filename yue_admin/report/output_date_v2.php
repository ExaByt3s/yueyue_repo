...
$fileName = "date";
$headArr = array ("订单号ID","报名ID","活动ID","地区","约拍状态", "摄影师ID", "摄影师昵称", "摄影师手机", "模特ID", "模特昵称", "模特手机", "拍摄风格", "时长", "价格","优惠", "付款状态", "付款时间", "拍摄时间", "拍摄地点", "签到状态", "活动状态","机构","是否入库" );
...
$date_list = get_all_event_date ( false, $where_str, "date_id DESC", '0,5000' );
...
echo "<table>";
foreach($new_date_list as $key=>$val)
{
    echo "<tr>";
    echo "<td>{$val['date_id']}</td>";
    echo "<td>{$val['enroll_id']}</td>";
    echo "<td>{$val['event_id']}</td>";
    echo "<td>{$val['location_name']}</td>";
    echo "<td>{$val['date_status']}</td>";
    echo "<td>{$val['from_date_id']}</td>";
    echo "<td>{$val['cameraman_nickname']}</td>";
    echo "<td>{$val['cameraman_cellphone']}</td>";
    echo "<td>{$val['to_date_id']}</td>";
    echo "<td>{$val['model_nickname']}</td>";
    echo "<td>{$val['model_cellphone']}</td>";
    echo "<td>{$val['style']}</td>";
    echo "<td>{$val['hour']}</td>";
    echo "<td>{$val['date_price']}</td>";
    echo "<td>{$val['discount_price']}</td>";
    echo "<td>{$val['pay_status']}</td>";
    echo "<td>{$val['pay_time']}</td>";
    echo "<td>{$val['date_time']}</td>";
    echo "<td>{$val['date_address']}</td>";
    echo "<td>{$val['is_checked']}</td>";
    echo "<td>{$val['event_status']}</td>";
    echo "<td>{$val['is_checked']}</td>";
    echo "<td>{$val['org_name']}</td>";
    echo "<td>{$val['is_set']}</td>";
    echo "</tr>";
}
echo "</table>";
