...
*/
    ...
    check_authority(array('cameraman'));
 	...
 	$tpl = new SmartTemplate("cameraman_honor_show.tpl.htm");
 	$page_obj = new show_page ();
 	$show_count = 20;
 	$uid = $_INPUT['uid'] ? $_INPUT['uid'] : 0;
 	$where_str = "uid = {$uid}";
    $page_obj->setvar (array('uid' => $uid));
 	$total_count = $cameraman_add_obj ->get_cameraman_honor_list(true, $where_str);
    $page_obj->set ( $show_count, $total_count );
    $list = $cameraman_add_obj->get_cameraman_honor_list(false, $where_str, $order_by = 'uid DESC', $page_obj->limit(), $fields = '*');
