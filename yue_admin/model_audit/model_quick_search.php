...
$recycle = 0;
$act     = trim($_INPUT['act']) ? trim($_INPUT['act']) : 'list';
...
$start_join_time =  intval($_INPUT['start_join_time']);
$end_join_time   =  intval($_INPUT['end_join_time']);
...
$hour            =  trim($_INPUT['hour']) ? trim($_INPUT['hour']) : 'twoh_price';
...
//码数
if($start_shoe_size>0)
{
    $where_str .= " AND shoe_size >= {$start_shoe_size}";
    $setParam['start_shoe_size'] = $start_shoe_size;
}
if($end_shoe_size>0)
{
    $where_str .= " AND shoe_size <= {$end_shoe_size}";
    $setParam['end_shoe_size'] = $end_shoe_size;
}
//加入时间
if($start_join_time >0)
{
    $where_str .= " AND left(p_join_time,4) >= {$start_join_time}";
    $setParam['start_join_time'] = $start_join_time;
}
if($end_join_time >0)
{
    $where_str .= " AND left(p_join_time,4) <= {$end_join_time}";
    $setParam['end_join_time'] = $end_join_time;
}
//身高
if($min_height >0)
{
    $where_str .= " AND height >= {$min_height}";
    $setParam['min_height'] = $min_height;
}
if($max_height >0)
{
    $where_str .= " AND height <= {$max_height}";
    $setParam['max_height'] = $max_height;
}
//年龄
if($min_age >0)
{
    $where_str .= " AND (YEAR(NOW()) - left(age,4)) >={$min_age}";
    $setParam['min_age'] = $min_age;
}
if($max_age >0)
{
    $where_str .= " AND (YEAR(NOW()) - left(age,4)) <={$max_age}";
    $setParam['max_age'] = $max_age;
}
...
if($min_price >0)
{
    $style_where_str .= " AND {$hour} >= {$min_price}";
    $setParam['min_price'] = $min_price;
}
if ($max_price >0)
{
    $style_where_str .= " AND {$hour} <= {$max_price} ";
    $setParam['max_price'] = $max_price;
}
...
{
    $setParam['join'] = $join;
    $j_where_str = "activity_join = '".mysql_escape_string($join)."'";
...
if (strlen($enter) >0)
{
    $setParam['enter'] = $enter;
    $e_where_str = "activity_enter = '".mysql_escape_string($enter)."'";
...
if($min_weight >0)
{
    $where_str .= " AND weight >= {$min_weight}";
    $setParam['min_weight'] = $min_weight;
}
if($max_weight >0)
{
    $where_str .= " AND weight <= {$max_weight}";
    $setParam['max_weight'] = $max_weight;
}
//状态
if ($p_state >0)
{
    $where_str .= " AND  state = {$p_state} ";
    $setParam['p_state'] = $p_state;
}
//罩杯
if ($cup_id>0)
{
    $where_str .= " AND  cup_id = {$cup_id}";
    $setParam['cup_id'] = $cup_id;
}
if ($cup_a>0)
{
    $where_str .= " AND cup_a ={$cup_a}";
    $setParam['cup_a'] = $cup_a;
}
...
//进学校年份
if (strlen($p_enter_school_time) >0)
{
    $where_str .= " AND p_enter_school_time = '".mysql_escape_string($p_enter_school_time)."'";
    $setParam['p_enter_school_time'] = $p_enter_school_time;
}
//录入者
if ($inputer_id >0)
{
...
//录入时间
if(strlen($start_inputer_time) >0)
{
    $where_str .= " AND FROM_UNIXTIME(UNIX_TIMESTAMP(inputer_time),'%Y-%m-%d') >= '".mysql_escape_string($start_inputer_time)."'";
    $setParam['start_inputer_time'] = $start_inputer_time;
}
if(strlen($end_inputer_time) >0)
{
    $where_str .= " AND FROM_UNIXTIME(UNIX_TIMESTAMP(inputer_time),'%Y-%m-%d') <= '".mysql_escape_string($end_inputer_time)."'";
    $setParam['end_inputer_time'] = $end_inputer_time;
}
//学校
if (strlen($p_school) >0)
{
    $where_str .= " AND p_school like '%".mysql_escape_string($p_school)."%'";
    $setParam['p_school'] = $p_school;
}
//专业
if (strlen($p_specialty) >0)
{
    $where_str .= " AND p_specialty like '%".mysql_escape_string($p_specialty)."%'";
    $setParam['p_specialty'] = $p_specialty;
}
//积分
if($min_score >0)
{
    $where_str .= " AND total_score >= {$min_score}";
    $setParam['min_score'] = $min_score;
}
if($max_score >0)
{
    $where_str .= " AND total_score <= {$max_score}";
    $setParam['max_score'] = $max_score;
}
//来源
if (strlen($information_sources)>0)
{
    $where_str .= " AND information_sources like '%".mysql_escape_string($information_sources)."%'";
    $setParam['information_sources'] = $information_sources;
}
//标签
if (!empty($label))
{
