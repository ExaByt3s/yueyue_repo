 ...
 */
    ...
    $tpl     = new SmartTemplate("model_quick_search_v2.tpl.htm");
    ...
    $start_shoe_size     =  trim($_INPUT['start_shoe_size']);
    $end_shoe_size       =  trim($_INPUT['end_shoe_size']);
    $start_join_time     =  trim($_INPUT['start_join_time']);
    $end_join_time       =  trim($_INPUT['end_join_time']);
    $min_height          =  trim($_INPUT['min_height']);
    $max_height          =  trim($_INPUT['max_height']);
    $min_age             =  trim($_INPUT['min_age']);
    $max_age             =  trim($_INPUT['max_age']);
    $min_price           =  trim($_INPUT['min_price']);
    $max_price           =  trim($_INPUT['max_price']);
    $hour                =  trim($_INPUT['max_price']);
    $min_weight          =  trim($_INPUT['min_weight']);
    $max_weight          =  trim($_INPUT['max_weight']);
    $p_state             =  trim($_INPUT['p_state']);
    $start_follow_time   =  trim($_INPUT['start_follow_time']);
    $end_follow_time     =  trim($_INPUT['end_follow_time']);
    $cup_id              =  trim($_INPUT['cup_id']);
    $cup_a               =  trim($_INPUT['cup_a']);
    $style               =  isset($_INPUT['style']) ? intval($_INPUT['style']) : -1;
    $join                =  trim($_INPUT['join']);
    $enter               =  trim($_INPUT['enter']);
    $grade               =  trim($_INPUT['grade']);
    ...
    $min_score           =  trim($_INPUT['min_score']);
    ...
    //码数
    if(!empty($start_shoe_size))
    {
        $where_str .= " AND shoe_size >= {$start_shoe_size}";
    }
    if (!empty($end_shoe_size)) 
    {
    ...
    if ($org_user_id != -1) 
    {
        ...
        $org_uid       = $org_relate_obj->get_model_org_list_by_org_id($b_select_count = false,$org_where_str, $limit = '0,10000', $order_by = 'id DESC',  $fields = 'user_id');
        //print_r($org_uid);
        if (empty($org_uid)) 
        {
            $where_str_list .= " AND uid = {$tmp_user_id}";
    ...
    $uiid = $model_add_obj->get_search_uid($where_str,'0,10000', 'uid DESC', 'DISTINCT(uid)' );
    if (!empty($uiid)) 
    {
