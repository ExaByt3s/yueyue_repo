...
function list_all_yue($url)
{
	...
	({
        ...
        success:function(data)
        {
        	...
        	for (var i = 0; i < $list.length; i ++) 
        	{
        		$str += "<tr><td align='center'>"+$list[i]['date_time']+"</td><td align='center'>"+$list[i]['cameraman_nick_name']+"</td><td align='center'>"+$list[i]['date_style']+"</td><td align='center'>"+$list[i]['date_address']+"</td><td align='center'><a href='javascript:void(0);' class='desc' data='"+$list[i]['comment_text']+"'>"+$list[i]['comment_desc']+"</td><td align='center'>"+$list[i]['date_price']+"</td></tr>";
        	}
        	$("#yue_data").append($str);
        	$.text_show();
        	$.text_hide();
