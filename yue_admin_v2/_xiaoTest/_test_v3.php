...
foreach($user_list as $v)
{
    ...
    $regist_time =   date('Y',strtotime($v['regist_time'])) == '1970' ? '--' : $v['regist_time'] ;
    $last_login_time = date('Y',strtotime($v['last_login_time'])) == '1970' ? '--' : $v['last_login_time'] ;
    echo "<tr>";
    echo "<td>{$v['user_id']}</td>";
    echo "<td>{$v['regist_time']}</td>";
    echo "<td>{$regist_time}</td>";
...
function get_interface_user_list($type_id)
{
    ...
    $sql_str = "SELECT DISTINCT(user_id),regist_time,last_login_time FROM yueyue_interface_db.interface_data_tbl WHERE type_id={$type_id} AND user_id !=0";
    $result = db_simple_getdata($sql_str, false, 22);
