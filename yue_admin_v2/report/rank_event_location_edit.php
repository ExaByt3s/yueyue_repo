...
check_auth_v2($yue_login_id,'rank_event_location_edit');//获取基本权限

$cat_list = include_once('../rank/cat_config.inc.php');//分类配置
$versions_list = include_once('../rank/versions_config.inc.php');//版本配置
...
$tpl = new SmartTemplate(REPORT_TEMPLATES_ROOT."rank_event_location_edit.tpl.htm");
...
if($act == 'insert')//插入
{
    check_auth_v2($yue_login_id,'rank_event_location_edit',$location_id);//有无插入权限
    ...
    if($retId >0)
    {
        js_pop_msg_v2('添加成功',false,'rank_event_location_list.php');
    }
    js_pop_msg_v2('添加失败');
}
elseif($act == 'edit')//修改
...
elseif($act == 'update')//更新数据
{
    ...
    if($retId >0)
    {
        js_pop_msg_v2('更新成功',false,'rank_event_location_list.php');
    }
    js_pop_msg_v2('更新失败');
}
elseif($act == 'del')
