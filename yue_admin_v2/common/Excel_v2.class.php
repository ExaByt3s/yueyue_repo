    ...
    public static function start($head,$body,$filename='',$headStyle='',$bodyStyle='')
    {
        ...
        $agent = $_SERVER['HTTP_USER_AGENT'];
        {
            ...
            if (preg_match('/win/i', $agent))
            {
                ...
                include_once("/disk/data/htdocs232/poco/pai/yue_admin_v2/common/Excel_v3.class.php");//导出类
                Excel_v3::start($head,$body,$filename);
                exit;
            }
        }
        //头
    ...
    public function addHeader($array){
        ...
        foreach ($array as $k => $v){
            ...
            if(is_array($v)){//是数组的时候
                $title = trim($v['title']);
                $width = intval($v['width']) == 0 ? 100:intval($v['width']);
            }else{
                $title = trim($v);
            }
