...
$tpl = new SmartTemplate( AUDIT_TEMPLATES_ROOT.'pic_del_examine_list.tpl.htm' );
...
$total_count = $pic_examine_v2_obj->get_pic_del_list(true,$date,$act,$audit_id,$user_id,$where_str);
$page_obj->set($show_page,$total_count);

$list = $pic_examine_v2_obj->get_pic_del_list(false,$date,$act,$audit_id,$user_id,$where_str,'audit_time DESC,id DESC',$page_obj->limit());
...
if($act == 'works')
{
    $setParam['title'] = "已删除作品";
}
elseif($act == 'merchandise')
{
    $setParam['title'] = '已删除商城图片';
}
else
{
    $setParam['title'] = "已删除头像";
}
