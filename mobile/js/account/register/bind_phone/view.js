...
define(function(require, exports, module)
{
    ...
    var bind_phone_tpl = require('./tpl/main.handlebars');
    var utility = require('../../../common/utility');
    var Scroll = require('../../../common/scroll');
    var App = require('../../../common/I_APP');
    var m_alert = require('../../../ui/m_alert/view');

    var bind_phone_view = View.extend
    ({

        attrs:
        {
            template: bind_phone_tpl
        },
        events :
        {
            'swiperight' : function ()
            ...
            'tap [data-role="get_code"]' : function()
            {
                ...
                self.model.bind_phone_code
                ({
                    phone : self.enter_phone.val(),
                    type : 'bind_sent_verify_code'
                })
            },
            'tap [data-role="bind_phone"]' : function()
            {
                var self = this;
        ...
        _setup_events : function()
        {
            ...
            self.model
                .on('success:code',function(response,options)
                {
                ...
                .on('complete:code',function(response,options)
                {
                ...
                .on('success:bind_phone',function(response,options)
                {
                    m_alert.show(self.model.toJSON().msg,'right');
                    ...
                    if(self.model.toJSON().code == 1)
                    {
                        ...
                        var icon = response.result_data.data.user_icon;

                        var params =
                        {
                            pocoid : response.result_data.data.user_id,
                ...
                .on('complete:bind_phone',function(response,options)
                {
                    //m_alert.hide({delay : 1000})
                })
                .on('error:bind_phone',function(response,options)
                {
                    m_alert.show(self.model.toJSON().msg,'error');
    ...
    module.exports = bind_phone_view;
});