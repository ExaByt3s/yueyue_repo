...
define(function(require, exports, module)
{
    ...
    var item = require('./tpl/item.handlebars');
    ...
    ({
        ...
        events :
        {
            ...
            'tap [order-id]' : function (ev)
            {
                var self = this;
                var order_id = $(ev.currentTarget).attr('order-id');
                page_control.navigate_to_page('camera_demand/detail/'+order_id); 
            ...
            'tap [data-carry]' : function(ev)
            {
                ...
                switch (self.current_type)
                {
                    case 'style' :
                        ...
                        self.$('[data-role="style"]').html(query_html);
                        break;
        ...
        _setup_events : function()
        {
            ...
            {
                var view_port_height = self.reset_viewport_height()- 40;
        ...
        _render_act_list: function(response,xhr)
        {
            ...
            console.log(response.result_data.list); 


            var ele_html = item({
                data : response.result_data.list
            })

            var method = xhr.reset ? 'html' : 'append';

            self.$list_container[method](ele_html);
...
//             });
        ...
        setup : function()
        {
            ...
            //进行抛包
            var img = new Image();
            img.src = 'http://yp.yueus.com/mobile_app/touch/oa_last_visit.php?location_id='+utility.storage.get("location_id");
