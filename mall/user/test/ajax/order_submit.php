...
$prices_type_id = trim($_INPUT['prices_type_id']);
...
$service_cellphone = intval($_INPUT['service_cellphone']);
...
$goods_type = $goods_info['goods_data']['type_id'] == 42 ? 'activity' : 'normal';
...
$__is_weixin = MALL_UA_IS_WEIXIN;
$__is_yueyue_app = MALL_UA_IS_YUEYUE; 
...
switch ($goods_type) {
	...
	case 'activity':
		 ...
		 * 	'activity_id' => 0, //活动ID
		 *	‘stage_id’=>0, //场次ID
		 * 	'prices_type_id' => 0, //规格ID
		 *  'service_cellphone' => '', //报名活动电话
		 * 	'quantity' => 0, //报名数量
		...
		$activity_list[] = array(
		   ...
		   'service_address' => $service_address,
		   'quantity'=>$quantity,
		   'activity_promotion_id'=>$promotion_id
		);	

		$ret = $mall_order_obj->submit_order_activity($yue_login_id, $activity_list, $more_info);
		break;
}
