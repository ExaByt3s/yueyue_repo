...
// 读取页面
$tpl = $my_app_pai->getView(TASK_TEMPLATES_ROOT.$pc_wap.'order/confirm-test.tpl.htm');
...
if($prices_list_de)
{
	...
	else
	{
		...
		foreach($prices_list_de as $key_de => $val_de)
		{
			...
			if(!empty($prices_list_data))
			{
				foreach ($prices_list_data as $p_key => $p_value) 
				{
					...
					$temp_arr = array(
						...
						'price_name' => $p_value['prices'],
						'selected' => $type_btn_selected,
						'stage_id' => $val_de['id'],
						'show' => $show,
						'goods_prices' => $p_value['value']
					);
			...
			$time  = date("Y.m.d H:i",$val_de['time_s']).' - '.date("H:i",$val_de['time_e']);
