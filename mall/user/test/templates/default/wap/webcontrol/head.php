...
// 跳转对应的域名
if(preg_match("/www.yueus.com/", $_SERVER['SERVER_NAME']) && MALL_UA_IS_MOBILE == 1)
{
        $url='http://'.'yp.yueus.com'.$_SERVER["REQUEST_URI"]; 
        header("Location:{$url}");
        exit();
}
elseif(preg_match("/yp.yueus.com/", $_SERVER['SERVER_NAME']) && MALL_UA_IS_PC == 1)
{
        $url='http://'.'www.yueus.com'.$_SERVER["REQUEST_URI"]; 
        header("Location:{$url}");
        exit();
}
...
{	
	...
	$js_debug = trim($_GET['js_debug']);


	$tpl->assign('js_debug',$js_debug);
