...
if(mode.trim() =='is_pub' || mode.trim() =='is_pub_test')
{
	...
	var common_js_prefix = '/modules/common/';
	var common_js_map = ['cookie/index.js','json/json.js','ua/index.js','utility/index.js','valid/index.js','global-top-bar/global-top-bar.js','widget/**.js'];

	for(var i = 0;i<common_js_map.length;i++)
	{
		common_js_map[i] = common_js_prefix + common_js_map[i];
	}

	fis.config.set('pack', 
	{
		//打包输出为common.js ,数组里面的匹配为要指定打包的文件
		'js/common/common.js' : common_js_map,
		//打包输出为common.css ,数组里面的匹配为要指定打包的文件
		'style/libs/common.css' : ['/style/common/**.scss']
	});
}
