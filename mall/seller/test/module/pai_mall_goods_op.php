...
/*******美食达人特殊处理2015-10-20********/
/*******约活动地区选择2015-11-4********/
$locate_value_checked = (int)$_INPUT['locate_value_checked'];
/*******约活动地区选择2015-11-4********/
...
if(!empty($_INPUT['default_data']))
{
    ...
    foreach($_INPUT['default_data'] as $key => $value)
    {
        if(empty($value))
        {
            ...
            //摄影服务没有图文内容
            //约活动选择全国，可以没有地区数据2015-11-4
            if($key=="location_id")
            {
                //约活动特殊处理
                if($type_id==42 && $locate_value_checked==0)
                {
                    $_INPUT['default_data'][$key] = 0;//选择全国为0
                    continue;
                }

                //约有趣商品类可不设地址2015-11-19，特殊处理
                if($type_id==43 && $_INPUT['system_data']["a8abb4bb284b5b27aa7cb790dc20f80b"]=="c203d8a151612acf12457e4d67635a95")
                {
                    continue;
                }
        ...
        else
        {
            ...
            if($key=="content")
            {
                ...
                //检测服务详情图片张数
                /*$content_img_len = mall_src_link_len_check($_INPUT['default_data'][$key]);
                if($content_img_len>5)
                {
                    $data_error = "服务详情图片张数不能超过20张，";
                    $data_error_tips = "data_error";
                }*/
                //检测服务详情图片张数结束
...
if($type_id==42)
{
    $prices_diy_tmp = $_POST['prices_diy'];
    //过滤处理
    foreach($prices_diy_tmp as $k => $v)
    {
        $prices_diy_tmp[$k]["name"] = mall_remove_xss($v["name"]);
        $prices_diy_tmp[$k]["time_s"] = mall_remove_xss($v["time_s"]);
        $prices_diy_tmp[$k]["time_e"] = mall_remove_xss($v["time_e"]);
        $prices_diy_tmp[$k]["stock_num"] = mall_remove_xss($v["stock_num"]);
        ...
        $prices_diy_tmp[$k]["detail"] = $v["detail"];


    }
    /*if($yue_login_id==100004)
    {
        print_r($prices_diy_tmp);
...
if($action=="add")
{
    ...
    else
    {
        if($type_id==42)
        {
            echo "<script>top.window.layer.closeAll();top.window.location.href='../activity_list.php?show=4'</script>";
        }
        else
        {
...
else if($action=="edit")
{
    ...
    else if($type_id==42)//活动的编辑入库特殊处理
    {

        //print_r($op_data);
        //exit();
        $res = $pai_mall_goods_obj->user_update_goods_for_42($goods_id,$op_data,$user_id);
        /*if($user_id==100004)
        {
            print_r($op_data);
            print_r($res);
...
else if($action=="money_edit")
{
    ...
    else
    {
        ...
        if($type_id==42)
        {
            echo "<script>top.window.layer.closeAll();top.window.location.href='../activity_list.php?show=1'</script>";
        }
        else
        {
            //echo "insert success";
