define('common/upload/1.0.0/yueyue_uploader', function(require, exports, module){ /**
 * Created by hudingwen on 15/6/10.
 */

 /*
	@require modules/common/upload/1.0.0/css/style.css
 */

require('components/jquery/jquery.js');
require('common/json/json');
require('common/sortable/sortable');
var cookie = require('common/cookie/index');
var utility = require('common/utility/index');

if(/test/.test(window.location.href))
{
	var current_url = 'http://static.yueus.com/mall/seller/test/static/pc/modules/common/upload/1.0.0/yueyue_uploader.js'.split('/');
}
else
{
	var current_url = 'http://static-c.yueus.com/mall/seller/static/pc/modules/common/upload/1.0.0/yueyue_uploader.js'.split('/');
}
        ...
        add_item_img : function($el,params)
        {
            ...
            if(self.options.upload_container)
			{
				var add_img_tpl = '<div class="ui-btn-add ui-btn disable-drag" data-role="add-btn">'+
                    '<a href="javascript:void(0);" class="ui-bf" title="添加图片" id="popup_'+self.options.gid+'"></a>'+
                    '</div>';
				...
				if(imgs.length>0)
				{
					for(var i=0;i<imgs.length;i++)
					{
                        ...
                        if(self.options.upload_logic_key == 'front')    
                        {                            
                            url = utility.matching_img_size(url,320);               
                        }                            
						...
						var tpl = '<div class="ui-upload ui-btn bg-color" data-role="ui-success-upload-img"  >'+
								...
								'   <input type="hidden" name="yue_upload_group_'+self.options.gid+'[]" value="'+url+'" />' +
								'</a>'+
