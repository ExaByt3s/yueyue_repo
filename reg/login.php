...
$main_url = "http://www.yueus.com/pc/login.php?r_url=".$param_r_url;
...
if(!empty($r_url) || $r_url!="")
{
    ...
    if($parse_url_arr['scheme']!="http")
    {
        echo "<script>
               alert('返回链接不符合链接结构');
               location.href='http://www.yueus.com/reg/login.php';
            </script>";
        exit();
    }
...
if(!empty($yue_login_id))
{
    ...
    /*if(preg_match("/task\.yueus/",$_SERVER['HTTP_REFERER']))
    {
        header("location:http://task.yueus.com/");
    }
    else
    {*/
...
$tpl->assign('encode_r_url',urlencode($r_url));
