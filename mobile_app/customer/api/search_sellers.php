...
//    $total_overall_score = intval($seller['total_overall_score']);
//    $review_times = intval($seller['review_times']);
//    $score = 5;
//    if ($review_times > 0) {
//        $score = intval($total_overall_score / $review_times);
//    }
    $cover = empty($seller['cover']) ? $default_cover : $seller['cover'];
    $size = strpos($cover, '.poco.cn') === false ? 260 : 640;
    $service_info = array(
        'user_id' => $seller_id, // 商家ID
        'name' => $seller['name'], // 商家名称
        'cover' => yueyue_resize_act_img_url($cover, $size), // 背景图
        'type_id' => trim($seller['type_id'], ','), // 分类
        'score' => strval(sprintf('%.1f', $seller['total_average_score'])), // 评分
        'trade_num' => '交易次数：' . intval($seller['seller_bill_pay_num']) . '次',
