...
if (count($pro_prices_list) > 1) {
    ...
    $show_prices = empty($prices_unit) ? $goods_prices : $goods_prices . '/' . $prices_unit;
...
// 下单URL
$order_url = 'http://yp.yueus.com/mall/user/order/confirm.php?goods_id=' . $goods_id . '&type_id=' . $type_id . '&direct_order_id=' . $direct_order_id;
if (version_compare($version, '3.3', '>')) {
    // for test (活动)
    $order_url = 'http://yp.yueus.com/mall/user/test/order/confirm.php?goods_id=' . $goods_id . '&type_id=' . $type_id . '&direct_order_id=' . $direct_order_id;
}
...
$goods_info = array(
    ...
    'rate' => '￥' . $goods_prices,
    'unit' => empty($prices_unit) ? '' : $prices_unit,
...
if ($type_id == 42) {
    ...
    $show_attend = $exhibit['attend'];  // 活动参与
    if (!empty($show_attend)) {
        // 修改 活动默认显示的价格信息
        $goods_info['prices'] = $show_attend['prices'] . $show_attend['unit']; //
        $goods_info['rate'] = $show_attend['prices'];
        $goods_info['unit'] = $show_attend['unit'];
    }
    $goods_info['showing'] = array(
        'title' => '活动场次',
        'more_title' => '查看更多场次',
        'is_finish' => $exhibit['is_finish'],  // 0进行中 ，1 已结束
        'attend' => $exhibit['attend'],
    ...
    if (empty($exhibit['showing_exhibit'])) {  // 全部结束
        ...
        $goods_info['showing']['attend'] = $show_attend;
        $goods_info['show_str'] = $is_show == 2 ? '已下架' : '已结束';  // 活动
    } elseif ($exhibit['is_finish'] == 1) { // 全部场次已报满
        $goods_info['show_str'] = $is_show == 2 ? '已下架' : '名额已满';  // 活动
    }
    // 报名名单
    $roster_list = array();
