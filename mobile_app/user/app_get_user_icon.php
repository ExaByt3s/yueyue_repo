...
foreach ($poco_id_arr as $k => $user_id) {
    ...
    if ($user_id == 10002) {
        $ret[$k]['url'] = "http://yp.yueus.com/mobile/app#account/about";
        $ret[$k]['wifi_url'] = "http://yp-wifi.yueus.com/mobile/app#account/about";
    } else {
    ...
    $user_info = $obj->get_user_info_by_user_id($user_id);
    if ($user_info['user_level']) {
        $ret[$k]['user_level'] = $user_info['user_level'];
    }
    $ret[$k]['city_name'] = iconv('gbk', 'utf-8', $user_info['city_name']);
}
