...
// 价格范围
if (count($pro_prices_list) > 1 && bccomp($min, $max, 2) != 0) {
    $min = sprintf('%.2f', $min);
    $max = sprintf('%.2f', $max);
    ...
    $show_prices = $goods_prices . $prices_unit;
}
...
if ($type_id == 42) {
    ...
    // 查看场次
    $exhibit = interface_get_goods_showing($prices_data_list, $goods_id, true);
    ...
    if (!empty($show_attend)) {
        // 修改 活动默认显示的价格信息
        $goods_info['prices'] = $show_attend['seller_prices']; //
        $goods_info['rate'] = $show_attend['seller_prices'];
        $goods_info['unit'] = '';
    }
    ...
    if (empty($exhibit['showing_exhibit'])) {  // 全部结束
        ...
        $goods_info['showing']['attend'] = $show_attend; // end($exhibit['all_exhibit']);
