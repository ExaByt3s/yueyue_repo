...
$goods_result = $task_goods_obj->user_get_goods_info($activity_id, $user_id);
...
$exhibit = interface_get_goods_showing($prices_data_list, $activity_id, true);
// 处理 场次
...
foreach ($trade_data as $value) {
    ...
    $trade_info = array(
        ...
        'phone' => $value['buyer_user_cellphone'],  // 买家手机号
        'standard' => $value['prices_spec'],  // 规格
        'buy_num' => $value['quantity'] . '人',  // 购买数量
        'remark' => $value['description'], // 备注
        'status' => $status,
        'status_str' => $status_str,
        'consult' => 'yueseller://goto?user_id=' . $user_id . '&receiver_id=' . $value['buyer_user_id'] . '&receiver_name=' .
            urlencode(mb_convert_encoding($value['buyer_user_name'], 'utf-8', 'gbk')) . '&pid=1250025&type=inner_app', // 聊天界面
    );
