...
$task_log_obj = POCO::singleton('pai_task_admin_log_class');  // 审核内容
$service_list = array();
foreach ($goods_list as $goods) {
    ...
    if ($type_id == 42) {
        ...
        $service_info = array(
            ...
            'prices' => '￥' . $price_str,
            'link' => 'yueseller://goto?user_id=' . $user_id . '&goods_id=' . $goods_id . '&pid=1220152&type=inner_app', // 跳转服务详情
            'total_showing' => '共' . intval($activity_result['total_show']) . '场',
            'showing_num' => $ing_show,
            'showing_desc' => '场进行中',
            'is_finish' => $ing_show > 0 ? 0 : 1,  // 0 可参与 1 结束
    ...
    $notice = '';
    if ($goods_status == 2) {  // 状态 0 未审核 1 通过 2未通过 3删除
