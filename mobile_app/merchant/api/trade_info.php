...
//        'request' => 'yueseller://goto?user_id=' . $user_id . '&receiver_id=' . $buyer_user_id . '&receiver_name=' . $buyer_user_name . '&pid=1250025&type=inner_app', // 聊天界面
    ),
    // 金额
    'cost' => array(
        'name' => '总价：',
...
//        array('title' => '余额', 'value' => $order_info['']),
...
if ($type_id == 42) {  // 活动订单
    $trade_data['trade'] = array(
        ...
        array('title' => '成交时间：', 'value' => date('Y.m.d H:i:s', $order_info['add_time'])),
    );
} else {
    $trade_data['trade'] = array(
        array('title' => '订单编号：', 'value' => $order_info['order_sn']),
...
//    $total_amount = ($total_amount == intval($total_amount)) ? intval($total_amount) : sprintf('%.2f', $total_amount);
    ...
    $pro_prices_list = array(
        'prices_type_id' => $goods_info['prices_type_id'], //必填
        'goods_prices' => $goods_info['prime_prices'], //必填
    );
