...
if (version_compare($version, '1.3', '<')) {
    // 处理 培训/咨询类型
    $train_type_arr = explode('-', $init_list['9fc3d7152ba9336a670e36d0ed79bc43']['option_value']);
    if (!empty($train_type_arr)) {
        $train_type_ = isset($train_type_arr[1]) ? $train_type_arr[1] : $train_type_arr[0];
        $init_list['9fc3d7152ba9336a670e36d0ed79bc43']['option_value'] = $train_type_;
    }
}else{
    // 线下授课 -> 招生范围(全国)
    if($init_list['44f683a84163b3523afe57c2e008bc8c']['option_value'] == '03afdbd66e7929b125f8597834fa83a4'){
        $init_list['location_id']['option_value'] = 0;
        $init_list['location_id']['value'] = '全国';
    }
}
$key_map = array(
    'common' => array(
        ...
        '9431c87f273e507e6040fcb07dcb4509' => 'other_tags',  // 其他培训类型
...
if (version_compare($version, '1.3', '>')) {
    // 1.3 版本, 无需 (旧)培训类型 / 课程类别
    unset($init_list['9fc3d7152ba9336a670e36d0ed79bc43'], $init_list['47d1e990583c9c67424d369f3414728e']);
    // 处理 招生范围
    $location_id_config = pai_mall_load_config('location_id');
    $location_id_arr = array();
    foreach ($location_id_config as $location) {
        $location_id_arr[] = array(
            'value' => $location['text'],
            'option_value' => $location['location_id'],
        );
    }
    $init_list['location_id']['options'] = $location_id_arr;
} else {
    // fix ios bug 2015-11-23
    unset($init_list['9431c87f273e507e6040fcb07dcb4509'],
        $init_list['4f6ffe13a5d75b2d6a3923922b3922e5'],
        $init_list['f5f8590cd58a54e94377e6ae2eded4d9'],
        $init_list['location_id'],
        $init_list['bbf94b34eb32268ada57a3be5062fe7d']);
}
        ...
        foreach ($tmp_second_options[$id] as $items) {
            ...
            if ($option_value == $match_id && !empty($merge_val)) {
                ...
                foreach ($merge_val as $val) {
                    // 处理 option_value, 方便入库
                    $tmp_items = $tmp_second_options[$val];
//                    foreach ($tmp_items as $tmp_item_) {
//                        $tmp_item_['option_value'] = $id . '-' . $option_value . '|' . $val . '-' . $tmp_item_['option_value'];
//                        $items_options[] = $tmp_item_;
//                    }
                    $items_options = array_merge($items_options, $tmp_items);
                }
