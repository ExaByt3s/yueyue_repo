...
$buyer_id = $client_data['data']['param']['buyer_id'];  // 买家 ID
...
$user_info = array(
    ...
    'name' => $name,
    'avatar' => get_user_icon($user_result['user_id'], 165), // 头像
    'location' => get_poco_location_name_by_location_id($user_result['location_id']),
    'introduce' => trim($introduce),
    // 聊天页面
    'chat' => array(
        'title' => '私聊TA',
        'request' => 'yueseller://goto?user_id=' . $user_id . '&receiver_id=' . $user_result['user_id'] .
            '&receiver_name=' . urlencode(mb_convert_encoding($name, 'utf8', 'gbk')) . '&pid=1250025&type=inner_app',
    ),
    // 属性
    'property' => array(
//        array('title' => '会员等级', 'value' => '13'),   // 会员等级暂时隐藏 2015-7-22 @荣少
    ...
    'business' => array(
        ...
        'merit' => array(
            'title' => '综合详情',
            'request' => 'yueseller://goto?user_id=' . $user_id . '&buyer_id=' . $user_result['user_id'] . '&pid=1250009&type=inner_app',
        ),
    ),
    'showtitle' => '精彩图集',
    // 作品展示
...
if ($is_display_record === 1) {
    ...
    foreach ($trade_data as $value) {
        ...
        if ($type_id == 20) {  // 面付
            ...
            $pid = 1250044;  // 活动详情
            $link = 'yueseller://goto?user_id=' . $user_id . '&goods_id=' . $goods_id . '&pid=' . $pid . '&type=inner_app'; // 跳转活动详情
        } else {
            $goods_id = $value['detail_list'][0]['goods_id'];
            $cover = yueyue_resize_act_img_url($value['detail_list'][0]['goods_images'], 260);
            $pid = 1250007;
            $link = 'yueseller://goto?user_id=' . $user_id . '&goods_id=' . $goods_id . '&pid=' . $pid . '&type=inner_app'; // 跳转服务详情
        }
        $trade_info = array(
...
$user_info['share'] = $mall_seller_obj->get_share_text($buyer_id);
