...
__poco_site_stat_obj.auto_mark_visit=function()
{
	...
	else
	{
		...
		try
		{
			this.writeCookie('visitor_flag',Math.round(new Date().getTime()/1000),24*365,'yueus.com');  //用户首次登录时间
			this.writeCookie('visitor_p', __poco_site_stat_request_filename.substr(0,__poco_site_stat_request_filename.indexOf('.css')) + '.css', 24*1, 'yueus.com');  //用户首次登录页面
			this.writeCookie('visitor_r',document.referrer,24*365,'yueus.com'); //用户首次登录的来源
		}
		catch (err)
		{
		}
	...
	this.writeCookie('visitor_b',this.readCookie('visitor_l'), null, 'yueus.com');

	this.writeCookie('visitor_l',__poco_site_stat_request_filename.substr(0,__poco_site_stat_request_filename.indexOf('.css')) + '.css', null, 'yueus.com');


	//补充会话session_id
	if (this.readCookie('session_id')=='')
	{
		this.writeCookie('session_id',Math.round(new Date().getTime()/1000), null, 'yueus.com');
	}
...
{
	__poco_site_stat_obj.write_code('window_unload.css','http://imgtj2.yueus.com/');
...
__poco_site_stat_obj.write_code = function(request_filename,tj_server)
{
	...
	{
		tj_server = 'http://imgtj.yueus.com/';
	}
	...
	if(request_filename == 'poco_tj.css')
	{
		...
		this.writeCookie('_current_time',new Date().getTime()/1000,0.05, 'yueus.com');
...
__poco_site_stat_obj.clickhead = function(ev)
{
		...
		if (typeof(tj_server)=='undefined')
		{
			tj_server = 'http://imgtj.yueus.com/click_hot.css';
		}
	...
	try
	{
		...
		if (window.top.location==window.location || typeof(_pocotj_ignore_block_frame)!='undefined')
		{
			__poco_site_stat_obj.write_code(__poco_site_stat_request_filename);
		  	...
		  	if(__poco_site_stat_request_filename == 'poco_tj.css' && window.location.host == 'my.yueus.com')
    		{
    			__poco_site_stat_obj.write_code(__poco_site_stat_request_filename);
    		}       
            
            if(__poco_site_stat_request_filename == 'poco_tj.css' && window.location == 'http://www.yueus.com/ams/res_mv.htm')
            {
                __poco_site_stat_obj.write_code(__poco_site_stat_request_filename);
            }
		}
...
{
	document.write('<script tyle="text/javascript" src="http://www1.yueus.com/poco_log_analyser/js/poco_spread.js"></'+'script>');
	var __spread_lock = 1;
}
...
{
	document.write('<script tyle="text/javascript" src="http://www1.yueus.com/poco_log_analyser/js/site_stat_clickheat.js"></'+'script>');
}

//热点图处理
if(window.top.location!=window.location && document.referrer.indexOf("my.yueus.com/site_stat/v3/click_hot.php") > -1)
{
