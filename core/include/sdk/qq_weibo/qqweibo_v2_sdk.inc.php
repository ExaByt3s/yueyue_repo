...
class QQT_V2_OAuth
{
     ...
     * @return string
     */
    public static function getAuthorizeURL($redirect_uri, $response_type = 'code', $wap = false , $state = '')
    {
        $params = array(
    ...
    {
        $r = json_decode(QQT_V2_Tencent::api('user/info'), true);
        if ($r['data']['name']) {
            return true;
        } else {
            self::clearOAuthInfo();
...
class QQT_V2_Tencent
{
	...
	/*
	 *  add by manson 2014.4.23
	 *  增加直接传递openid调用接口API
	 */
	public static function request_api($command , $openid , $access_token , $params = array() , $method = 'GET' , $format = "json" ,$multi = false)
	{
		//鉴权参数
		$params['access_token'] = $access_token;
		$params['oauth_consumer_key'] = QQT_V2_OAuth::$client_id;
		$params['openid'] = $openid;
		$params['oauth_version'] = '2.a';
		$params['clientip'] = QQT_V2_Common::getClientIp();
		$params['scope'] = 'all';
		...
		//请求接口
        $r = QQT_V2_Http::request($url, $params, $method, $multi);
    ...
    public static function api($command, $params = array(), $method = 'GET', $multi = false)
    {
        if (isset($_SESSION['t_access_token'])) {//OAuth 2.0 方式
            ...
            $params['appid'] = QQT_V2_OAuth::$client_id;
            $params['openid'] = $_SESSION['t_openid'];
            ...
            //生成签名
            $urls = @parse_url($url);
            $sig = QQT_V2_SnsSign::makeSig($method, $urls['path'], $params, QQT_V2_OAuth::$client_secret.'&');
            $params['sig'] = $sig;
        }
