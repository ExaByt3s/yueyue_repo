...
if($_GET["type"]=="close")
{
	mysql_query("UPDATE app_topic_msg SET valid=0 WHERE id='".$_GET["uid"]."'");
}
if($_GET["type"]=="open")
{
	mysql_query("UPDATE app_topic_msg SET valid=1 WHERE id='".$_GET["uid"]."'");
}

$res1 = mysql_query("SELECT count(*) FROM app_topic_msg");
...
$res = mysql_query("SELECT id,groupid,msg,valid FROM app_topic_msg order by id desc limit ".$offset.",".$pagesize);
while(list($id,$groupid,$msg,$valid)=mysql_fetch_array($res))
{
	if($valid==1)
	{
		$tpl->assign(
			array(
				...
				TITLE	=> $msg,
				GROUPID	=> $groupid,
				GOURL	=> "<a href='main.php?action=msgcontent&type=close&uid=".$id."'><font color='red'>ÆÁ±Î</font></a>"
			)
		);
	}
	else
	{
		$tpl->assign(
			array(
				UIDS	=> $id,
				TITLE	=> $title,
				GOURL	=> "<a href='main.php?action=msgcontent&type=open&uid=".$id."'><font color='bule'>»Ö¸´</font></a>"
			)
		);
	}
