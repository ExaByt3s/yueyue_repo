          ...
          if ($ftype == TType::MAP) {
            ...
            for ($_i44 = 0; $_i44 < $_size40; ++$_i44)
            {
              ...
              for ($_i51 = 0; $_i51 < $_size47; ++$_i51)
              {
                ...
                $xfer += $input->readI32($val53);
                $val46[$key52] = $val53;
              }
              $xfer += $input->readMapEnd();
              $this->facet[$key45] = $val46;
            }
            $xfer += $input->readMapEnd();
          } else {
            $xfer += $input->skip($ftype);
  ...
  public function write($output) {
    ...
    if ($this->resultRow !== null) {
      ...
      $xfer += $output->writeFieldBegin('resultRow', TType::LST, 4);
      {
        ...
        {
          foreach ($this->resultRow as $iter54)
          {
            {
              $output->writeMapBegin(TType::STRING, TType::STRING, count($iter54));
              {
                foreach ($iter54 as $kiter55 => $viter56)
                {
    ...
    if ($this->Keyword !== null) {
      ...
      $xfer += $output->writeFieldBegin('Keyword', TType::LST, 5);
      {
        ...
        {
          foreach ($this->Keyword as $iter57)
          {
    ...
    if ($this->groupCount !== null) {
      ...
      $xfer += $output->writeFieldBegin('groupCount', TType::MAP, 7);
      {
        ...
        {
          foreach ($this->groupCount as $kiter58 => $viter59)
          {
            $xfer += $output->writeString($kiter58);
            {
              $output->writeMapBegin(TType::STRING, TType::STRING, count($viter59));
              {
                foreach ($viter59 as $kiter60 => $viter61)
                {
    ...
    if ($this->facet !== null) {
      if (!is_array($this->facet)) {
        throw new TProtocolException('Bad type in structure.', TProtocolException::INVALID_DATA);
      }
      $xfer += $output->writeFieldBegin('facet', TType::MAP, 8);
      {
        $output->writeMapBegin(TType::STRING, TType::MAP, count($this->facet));
        {
          foreach ($this->facet as $kiter62 => $viter63)
          {
            $xfer += $output->writeString($kiter62);
            {
              $output->writeMapBegin(TType::STRING, TType::I32, count($viter63));
              {
                foreach ($viter63 as $kiter64 => $viter65)
                {
                  $xfer += $output->writeString($kiter64);
                  $xfer += $output->writeI32($viter65);
                }
              }
              $output->writeMapEnd();
            }
